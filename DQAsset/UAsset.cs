        ...
        public void Serialize(BinaryWriter uexp, BinaryWriter uasset, bool doFnameCleanup)
        {
            ...
            if (doFnameCleanup)
            {
                ...
                for (int i = Names.Count - 1; i >= 0; i--)
                    if (!Names[i].InUse)
                        if (!Names[i].Name.StartsWith("/Game/DataTables/") && Names[i].Name != "UserDefinedStruct") // some reason these isn't referenced by any FName in the file, make sure to exclude it...
